...
from ..common.behavior_cloning_model import  CNNModel
...
def steering_estimation(frame,model):
    next_frame = cv2.resize(frame, (1920, 1080))
    driving_window = next_frame[1080 - 600:1080 - 200, 20:800]
    driving_window = picasso.img_preprocess(driving_window)

    combined_image_test = cv2.normalize(driving_window, None, alpha=-1, beta=1, norm_type=cv2.NORM_MINMAX,
                                        dtype=cv2.CV_32F)
...
def load_data_from_txt(PATH_TRUE_SPEED):
...
def main():
    ...
    PATH_STEERING = common_constants.STEERING_ANGLE
    steering_data = load_data_from_txt(PATH_STEERING)
    ...
    while (front_cam.isOpened()):
        ...
        speed_data = str(int(3.6*float(velocity))) + '\n'
        file2.writelines(speed_data)
        ...
        picasso.draw_curved_points(frame, 100, float(steering_data[frame_count]), 300, 845, (255, 0, 0))
       ...
       # print(steering_angle)
        ...
        optical_flow = cv2.imread('/home/ahayouni/Documents/brite-unit2/src/speedEstimation/data/brite_reweighted_images_flow/'+str(frame_count)+'.jpg')
        optical_flow=cv2.resize(optical_flow,(484,304))

        combinedFrame[15+int(height / 4):15 + 2*int(height / 4), width - int(width / 4) - 10:width - 10] = optical_flow
